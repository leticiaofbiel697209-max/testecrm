    ...
    if pagina == "Ações de Hoje":
        ...
        with st.expander("Orçamentos com 2 dias"):
            if orc_2_dias.empty:
                ...
                tabela = orc_2_dias[[co_num, co_cli, co_valor, "dias_no_sistema"]].copy()
                tabela[co_valor] = tabela[co_valor].map(fmt)
                st.dataframe(tabela, use_container_width=True, hide_index=True)
        ...
        with st.expander("Orçamentos urgentes com 3+ dias"):
            if orc_urgentes.empty:
                ...
                tabela = orc_urgentes[[co_num, co_cli, co_valor, "dias_no_sistema", "acao_recomendada_orcamento"]].copy()
                tabela[co_valor] = tabela[co_valor].map(fmt)
                st.dataframe(tabela, use_container_width=True, hide_index=True)
        ...
        with st.expander("Clientes em atraso de recompra"):
            if atraso_recompra.empty:
                ...
                tabela = atraso_recompra[[
                    "Cliente", "Vendedor", "temperatura", "dias_sem_comprar",
                    "intervalo", "potencial_mensal"
                ]].copy()
                tabela["potencial_mensal"] = tabela["potencial_mensal"].map(fmt)
                st.dataframe(tabela, use_container_width=True, hide_index=True)
        ...
        with st.expander("Retornos programados para hoje"):
            if retornos_hoje.empty:
                ...
                tabela = retornos_hoje[["Cliente", "Vendedor", "retornos_hoje", "motivo_prioridade"]].copy()
                st.dataframe(tabela, use_container_width=True, hide_index=True)
